...
# ===== Database config =====
# Use DATABASE_URL (for Render) or fallback to local SQLite
sql_uri = os.getenv("DATABASE_URL") or os.getenv("SQLALCHEMY_DATABASE_URI")
if not sql_uri:
    # Fallback for local testing if no env variable is set
    sql_uri = "sqlite:///local.db"

# Fix old-style Postgres URL
